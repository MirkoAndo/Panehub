...
type Item = { productId: string | null; productName: string; quantity: number; unitPrice: number }
...
export async function POST(request: Request) {
  try {
    ...
    if (!name || !phone || !date || !slot || items.length === 0 || items.some((item) => !item.productName || !Number.isInteger(item.quantity) || item.quantity < 1 || item.quantity > 99 || !Number.isFinite(item.unitPrice) || item.unitPrice < 0)) return NextResponse.json({ error: 'Dati prenotazione non validi' }, { status: 400 })
    ...
    const { error: itemsError } = await supabase.from('reservation_items').insert(items.map((item) => ({ reservation_id: reservation.id, product_id: item.productId, product_name: item.productName, quantity: item.quantity, unit_price: item.unitPrice, subtotal: item.unitPrice * item.quantity })))
